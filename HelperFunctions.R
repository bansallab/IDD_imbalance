...
#### MY ADDITIONS ####
# from https://stackoverflow.com/questions/20169172/how-do-i-correctly-close-a-connection-in-r-so-its-connection-slot-gets-releas
...
get.cr.sep=function(x){
  #browser()
  if(is.null(x$family) & is.null(x$given)){ # if no given or family need to do something
    return(x$name)
  }else if(!is.null(x$given)){
...
# Some authors use a single name, and do not need a comma added
  ...
  for(i in missing){
    ...
    if(class(json_data)!="try-error"){
      if(!is.null(json_data$message$author)){
        ...
        names_sep=unlist(lapply(json_author,get.cr.sep)) # here
        # this causes problems for authors with single name
        # I excluded group authors in API pulls, so not sure if we need this conditional anymore
        # maybe should be if sum(is.na(names_sep)) == length(is.na(names_sep))
        #if(!is.na(names_sep)){ # my addition due to group author, introduces warning but i think ok
          #names_sep = names_sep[!is.na(names_sep)] # getting rid of NAs when no first or family name given
        Encoding(names_sep)="latin1"
        names_sep=replace_non_ascii(names_sep)
        #}
...
get.preferred=function(x){
  if(grepl("\\.",x)==TRUE & x!=toupper(x)){
    name=gsub("\\."," ",x) # remove periods
    name=strsplit(name," ")[[1]] # split up name
    notup=which(name!=toupper(name))
    name=name[notup][1] # which elements of name are not all uppercase, get the first
    # likely last name
    if(substr(name,1,1)!="-"){
      return(name)
    }else{
...
match.initials=function(x,allfirsts,alllasts,initials){
  if(initials[x]==F){
    return(target.first) # what is target.first?
  }else{
    target.first=allfirsts[x] # e.g. C. J. E.
    # remove punctuation to just get initials of first name, e.g. CJE
    target.initials=extract.initials(target.first) 
    target.last=alllasts[x] # last name, e.g. Metcalf
    # allfirsts doesn't keep track of frequency, it would be 74 to 1 Jessica vs Charlotte if so
    others=which(tolower(target.last)==tolower(alllasts) & 
                   initials==F) # potential matches to last name with full first name
    if(length(others)>0){ # other non-initials with same last name
      allsimilar.full=NULL
      allsimilar.concat=NULL
      allsimilar.clean=NULL
      for(j in others){
        samelast.full=allfirsts[j] # first name of potential match
        # remove punctuation & spaces in first name of potential match and make lowercase, e.g. cjessicae
        samelast.concat=tolower(gsub("[[:punct:][:blank:]]","",samelast.full))
        # get fullest name cleanly of potential match, e.g. Jessica
        ...
        if(samelast.full!=toupper(samelast.full) & 
          ...
          # same initials, and potential match isn't just initials
          # problematic, could be missing initial (WJ vs W Edmunds)
          # add first name of potential match to vector
          allsimilar.full=c(allsimilar.full,samelast.full)
          # add lower case first name to vector
          allsimilar.concat=c(allsimilar.concat,samelast.concat)
          # add cleaned last name to vector
          allsimilar.clean=c(allsimilar.clean,samelast.clean)
        }
      }
      # get unique potential matching first names, this should use full I think
      unique.clean=unique(allsimilar.clean) # was .clean
      # length of each name
      name.lengths=nchar(unique.clean)
      # pick longest name which is presumably most complete (problematic, Jessica vs Charlotte)
      # seems like it should be more about how common
      longest=unique.clean[which.max(name.lengths)]
      others=unique.clean[-which.max(name.lengths)]
      # see if longest is in others or others in longest?
      contained=unlist(lapply(others,grepl,longest))
      if(length(unique(allsimilar.concat))==1 |
         length(unique(allsimilar.clean))==1){
        # if all options the same, no disagreement
        ...
        # more than one option but all in longest option, no disagreement
        matched.name=sort(table(allsimilar.full),decreasing=T)
        matched.name=names(matched.name)[1]
        return(matched.name)
      }else{ # disagreement so don't change
        return(target.first)
      }
    }else{ # if no matching first names keep same last name
      return(target.first)
    }
  }
}
...
get.cited.indices=function(x,DI,CR){
  cited.split=strsplit(CR[x]," ")[[1]] # split on spaces
  cited.dois=which(cited.split=="doi")+1 # indices of entries after doi, so will have actual doi
  cited.dois=cited.split[cited.dois] # now get entry at those indices
  cited.dois=tolower(gsub(";|,|\\[|\\]","",cited.dois)) # replace weird chars in doi
  cited.dois=cited.dois[cited.dois!="doi"] # make sure no "doi" left by accident?
  # which DOIs in authorship are in this list of DOIs
...
get.ref.props=function(x,article.data,uncond_expecs){ #},cond_expecs){ # remove for now
  cited.papers=strsplit(article.data$CP[x],", ")[[1]] # list of cited papers by row num
  self.authored=strsplit(article.data$SA[x],", ")[[1]] # potential self-citations
  cited.notself=as.numeric(cited.papers[!(cited.papers%in%self.authored)]) # remove self-citations
  cited.notself=cited.notself[!is.na(article.data$GC[cited.notself])] # remove papers with no assigned author genders
  if(length(cited.notself)>0){ # if any non-self-citations in dataset
    cited.genders=article.data$GC[cited.notself] # get genders
    gender.table=table(factor(cited.genders,lev=0:3)) # make table of counts
    observed.props=gender.table/sum(gender.table) # convert to proportions
    uncond.exp.props=uncond_expecs[x,] # this is our expectation
    
    # commenting out due to lack of conditional expectations at the moment
    ...
    return(c(as.numeric(observed.props), # 4 of these, observed citation props of each gender
             as.numeric(uncond.exp.props), # 4 of these, random expected citation props of each gender
             # as.numeric(cond.exp.props), # 4 of these
             rep(NA, 4), # filler for no conditional props
             length(cited.notself))) # 1 of these, num non self citations
  }else{
...
get.prev.coauths=function(x,all_auth_names,month_from_base,year){
  # authors who have coauthored a paper with the authors of this paper
  ...
  this_date=date[x]
  earlier_than_this=(date<=this_date) # boolean vector whether article published before this one
  # I think I'd like this to be year
  
  auths_in=rep(FALSE,length(all_auth_names)) # set all to false
  
  sub_auth_names=all_auth_names[earlier_than_this] # names of authors published before
  # see if authors of interest are in any previous author lists
  # row is for each author I suspect, matches full name (not variants)
  auths_in_sub=sapply(seq_along(sub_auth_names),
                      function(y){this_auths %in% sub_auth_names[[y]]})
  if(!is.null(nrow(auths_in_sub))){
    auths_in_sub=apply(auths_in_sub,2,sum)>0 # condense, T/F per article
  }
  auths_in[earlier_than_this]=auths_in_sub # include earlier articles
  auths_in=which(auths_in) # coauthored article indices
  
  prev_coauths=unique(unlist(all_auth_names[auths_in])) # names of coauthors, I think can be middle authors too
  prev_coauths=prev_coauths[!(prev_coauths%in%this_auths)] # remove authors of this paper from list of coauthors (no self coauthorship)
  return(prev_coauths)
}
get.ma.overrep=function(x,prev_coauths,all_auth_names,
                        month_from_base,year,author_gends){
  # difference in proportion of male coauthors to proportion of male authors in field
  if(sum(is.na(month_from_base))==0){
    date=month_from_base
  }else if(sum(is.na(year))==0){
    date=year
  ...
  # genders of coauthors
  if(length(prev_coauth_gends)>0){
    pc_man_prop=sum(prev_coauth_gends=="M")/sum(prev_coauth_gends!="U")
    # prop that are male coauthors
    earlier_than_this=(date<=this_date)
    
    whole_field=unique(unlist(all_auth_names[earlier_than_this])) # all authors in field before this article
    whole_field_gends=author_gends$gend[author_gends$name%in%whole_field] # genders of all authors in field
    wf_man_prop=sum(whole_field_gends=="M")/sum(whole_field_gends!="U")
    # field man prop before this
    return(pc_man_prop-wf_man_prop) # returns difference, positive means work with more men than field
  }else{
    return(NA)
  }
}
get.mmp.overrep=function(x,prev_coauths,all_auth_names,
  ...
  # returns difference between proportion of coauthor's papers that are MM (so how often work with MM groups)
  # vs all eligible papers to cite that are MM
  if(sum(is.na(month_from_base))==0){
    date=month_from_base
  }else if(sum(is.na(year))==0){
    date=year
  ...
  sub_auth_names=all_auth_names[earlier_than_this] # eligible coauthors/papers list
  # pc_papers_sub is sequence of 1s and 0s if paper is written by coauthors for each eligible paper
  pc_papers_sub=sapply(seq_along(sub_auth_names), # seq from 1 to length of eligible papers/coauths list
                       function(x){prev_coauth %in% sub_auth_names[[x]]}) # coauthors papers, row for each coauth, 1 if authored by coauths
  if(is.null(nrow(pc_papers_sub))){ # why would this be null? all papers just have one author?
    pc_papers[earlier_than_this]=pc_papers_sub
    pc_papers=which(pc_papers>0) # indices of papers with a coauthor
  }else{
    pc_papers[earlier_than_this]=apply(pc_papers_sub,2,sum) # for papers before this one, sum coauthor columns to know how many coauthors on each eligible paper
    pc_papers=which(pc_papers>0) # indices of papers with a coauthor, no longer matters how many
  }
  
  if(length(pc_papers)>0){
    pc_paper_gends=article_gends[pc_papers] # genders of authors of coauthors' papers
    pc_mm_prop=sum(pc_paper_gends=="MM")/sum(!grepl("U",pc_paper_gends)) # prop that are MM
    # prop of coauthored papers that are MM, so how often work with MM groups
    all_paper_gends=article_gends[earlier_than_this]
    all_mm_prop=sum(all_paper_gends=="MM")/sum(!grepl("U",all_paper_gends)) # prop of all eligible papers MM
    
    return(pc_mm_prop-all_mm_prop) # returns difference of prop work with MM versus MM groups in pop
  }else{
    return(NA)
  }
}
...
citeprops=function(ref_proportions,i=NULL,type='conditional'){
  ...
  if(type=='conditional'){
    ...
    nas=apply(is.na(ref_proportions[i,c(1:8)]),1,sum)!=0 # change 8 to 12 when run GAM
    i=i[!nas]
    out=round(matrix(apply(ref_tot_sub[i,c(1:4,5:8)],2,sum)/ # sum down column with just eligible rows
...
citegap=function(ref_proportions,i=NULL,type='conditional'){
  ...
  if(type=='conditional'){
    ...
    out=apply(ref_proportions[i,1:4],2,sum)/ # col sums (that's why use total cites not props), take ratio - 1
...
f1plot=function(data,title,yl=T,xl=T, minyear=min(data$Year), maxyear=max(data$Year)){
  p=ggplot(data,aes(x=Year, y=Prop, fill=Color)) + 
    geom_area(alpha=0.9,linewidth=.5,color="black")+theme_bw()+
    scale_fill_identity()+
    scale_x_continuous(limits=c(minyear, maxyear),
                       breaks = seq(minyear - (minyear %% 5) + 5, maxyear, by = 5), 
                       expand=c(0,0))+
