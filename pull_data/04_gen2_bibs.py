...
# extract gen 2 bibs
# feed this file into python cited script to get gen2 bibs, set last value to 1 if first time running, 2 otherwise

os.chdir('../' + folder_for_runs + '/cited-reference-loop')
os.system("python3 wosExpandedAPICitedReferences.py input_get_gen2_bibs.csv output_get_gen2_bibs.csv get_gen2_bibs_completed_uts.csv 1")

print("Extracted generation 2 bibliographies")
os.chdir('../') # two folders up from cited-reference-loop is coreidd which is where we want to be
##############################################
...
ff.pull_full_records(input_filepath = "input_get_gen2_bibs_info.csv", 
                 ...
                 first_time = True, # if not True, be sure to have rerun the input file to shorten it
                 # ie provide completed and no records filepaths
                 completed_filepath = "completed_uts_gen2_bibs_info.csv",
                 norecords_filepath = "norecords_uts_gen2_bibs_info.csv")
