...
# have to do this after row_number call
...
sort(allfirsts[alllasts=="Edmunds"])
sort(allfirsts_matched[alllasts=="Edmunds"]) 
sort(allfirsts[alllasts=="Nokes"])
sort(allfirsts_matched[alllasts=="Nokes"]) 
sort(allfirsts[alllasts=="Metcalf"])
sort(allfirsts_matched[alllasts=="Metcalf"]) 
# This is not working for all names
...
saveRDS(article.data, file=paste0(folder_path, "/df5_articledata_matchednames.rds"))
